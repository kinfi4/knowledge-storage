...
# allows the use of the Any type in type hints

import tkinter as tk
# library for creating desktop applications

from tkinter import messagebox
# used to popup message dialog boxes for the user in a GUI

from tkinter.ttk import Treeview
# widget that allows to display a hierarchical data structure in a GUI
...
# imports several modules and objects required for an application 
# that involves handling and processing data related to products, customers, and orders.
...
class Window:                           # Class for 3 main windows (Customers, Products, Orders)
...
class CustomersWindow:                      # Window for Customers CRUD
    def __init__(self) -> None:
    ...
    def initialize_menu(self):          # initializing menu by destroying previous frames and creating new
        self.frame.destroy()
        self.frame = tk.Frame(bg=BACKGROUND)
        self.frame.pack()
    ...
    def update_customer(self):
        ...
        try:
            ...
            self.error_message(f"Can't update the record the record {str(err)}")
            return

        self.initialize_menu()

    def error_message(self, name, color=ERROR_COLOR):       # displaying an error message
        if self.error_label:
            self.error_label.destroy()

        self.error_label = tk.Label(
            self.entry_frame, text=name, bg=BACKGROUND, fg=color
        )
    ...
    def delete_customer(self):
        ...
        if answer == "yes":
            try:
                ...
                self.error_message("Can't delete the record")
                return
            self.initialize_menu()
        else:
...
class ProductsWindow:
    ...
    def delete_product(self):
        ...
        if answer == "yes":
            try:
                res = selected_record[PRODUCTS_COLUMNS[0]]
                print(f"FOUND: {res}")
                self._product_repository.delete(res)
            except Exception:
                self.error_message("Can't delete the record")
                return
            # refreshing all
    ...
    def error_message(self, name, color=ERROR_COLOR):
...
class OrdersMenu:
    ...
    def update_order(self):
        ...
        # everything is filled finally updating
            ...
            try:
                ...
                self.error_message(f"Impossible to add order. Please refer logs for more information: {str(error)}")
            else:
                # showing clear new window
                self.initialize_menu()
    ...
    def error_message(self, name, color=ERROR_COLOR):
        ...
        self.error_label = tk.Label(self.frame, text=name, bg=BACKGROUND, fg=color)
